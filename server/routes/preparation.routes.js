const router=require('express').Router(),
      Preparation=require('../models/preparation.model'),
      TimeLog=require('../models/timelog.model'),
      User=require('../models/user.model'),
      {verifyToken,isAdmin,canCreatePreparation}=require('../middleware/auth.middleware'),
...
router.post('/',verifyToken,canCreatePreparation,async(req,res)=>{
  try{
    ...
    const preparation=new Preparation({
      userId,timeLogId:activeTimeLog?activeTimeLog._id:null,agency,licensePlate,vehicleModel,status,notes,
...
router.post('/:id/tasks/:taskType/start',verifyToken,upload.single('photo'),async(req,res)=>{
  try{
    const{id,taskType}=req.params,{notes}=req.body;
    ...
    preparation.tasks[taskType].photos.before={url:req.file.path,timestamp:new Date()};
...
router.post('/:id/tasks/:taskType/complete',verifyToken,upload.single('photo'),async(req,res)=>{
  try{
    const{id,taskType}=req.params,{notes,amount,departureLocation,arrivalLocation}=req.body;
    ...
    preparation.tasks[taskType].photos.after={url:req.file.path,timestamp:new Date()};
...
router.post('/:id/tasks/:taskType/photos',verifyToken,upload.single('photo'),async(req,res)=>{
  try{
    const{id,taskType}=req.params,{description}=req.body;
    ...
    preparation.tasks[taskType].photos.additional.push({
      url:req.file.path,timestamp:new Date(),description:description||''
    });
...
router.put('/:id/complete',verifyToken,async(req,res)=>{
  try{
    const{id}=req.params,{notes}=req.body;
...
router.post('/:id/photos',verifyToken,upload.array('photos',5),async(req,res)=>{
  try{
    ...
    const photos=req.files.map(file=>({
      url:file.path,type,timestamp:new Date()
    }));
